...
namespace OMCL_DLL.Tools.Login.MicrosoftLogin.UI
{
    public partial class MicrosoftLogin : Form
    {
        ...
        private void MicrosoftLogin_Load(object sender, EventArgs e)
        {
            ...
            new OMCLLog("[MicrosoftLogin_Login]创建窗口： OMCL - 登录Microsoft 成功！", OMCLExceptionClass.DLL, OMCLExceptionType.Message);
        }
        private void WebBrowser1_Navigated(object sender, WebBrowserNavigatedEventArgs e)
        {
            ...
            new OMCLLog("[MicrosoftLogin_Login]已跳转，获取Code成功，Code隐藏。", OMCLExceptionClass.DLL, OMCLExceptionType.Message);
            IsLogin = true;
            Close();
        }
        private void button1_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                new OMCLLog("[MicrosoftLogin_Login]正在打开网页[" + url + "]，请稍后.....", OMCLExceptionClass.DLL, OMCLExceptionType.Message);
                Process.Start(url);
                Close();
            }
            catch
            {
                new OMCLLog("[MicrosoftLogin_Login]错误：打开网页时出现错误！该电脑可能没有浏览器！", OMCLExceptionClass.DLL, OMCLExceptionType.Error);
